...
# Exit on error.
set -o errexit
# Exit on error inside functions or subshells.
set -o errtrace
...
function __log () {
    ...
    local esc_code_after="0"
    local type_debug=("35" "   ↳")
    local type_info=("32" "=❱" )
    local type_notice=("34" "=▶")
    local type_success=("34" "√")
    local type_warning=("33" "⚠")
    local type_error=("31" "❎")
    local type_critical=("1;31" "❎")
    local type_alert=("1;33;41" "‼")
    local type_emergency=("1;4;5;33;41" "❎")

    local test_type="type_${level}"
    local type="${!test_type:-${type_error}}}"
    local log_esc_code="\xqb[${!log_esc_code:-${esc_code_error}}m"
    ...
    local icon_critical="❎"
    local icon_alert="‼"
    local icon_emergency="❎"
    local log_esc_code="icon_${level}"
    local log_icon="${!log_esc_code:-${icon_notice}}"



  # all remaining arguments are to be printed
  local log_prefix=$(printf "[%10s - %9s]\t" "$(date -u +"%d-%m-%Y %H:%M:%S")" "${level}")
  local log_line=""

  while IFS=$'\n' read -r log_line; do
    echo -e "$log_prefix ${message_style} ${} ${log_line} ${color_reset}" 1>&2
  done <<< "${@:-}"
}
